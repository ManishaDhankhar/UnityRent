...
const RentOrList = () => {
  return (
    <section className="py-20 px-4 bg-[#f4faf8]">
      <div className="max-w-6xl mx-auto text-center mb-12">
        <h2 className="text-4xl font-extrabold text-[#063b28] mb-4 tracking-tight">
          Two ways to join the mission
        </h2>
        <p className="text-[#2b4d40] text-lg font-medium">
      ...
      <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
        ...
        <MissionCard 
          ...
          buttonStyle="bg-[#063b28] text-white hover:bg-[#042b1d]"
          checkColor="text-[#063b28]"
        />
...
const MissionCard = ({ badge, badgeColor, title, description, image, features, buttonText, buttonStyle, checkColor }) => {
  return (
    <div className="bg-white rounded-3xl shadow-lg border border-[#e2e8e5] overflow-hidden flex flex-col h-full">
      {/* Image Header */}
      <div className="relative h-64">
        <img src={image} alt={title} className="w-full h-full object-cover" />
        <div className="absolute inset-0 bg-black/40 flex flex-col justify-end p-6">
          <span className={`${badgeColor} text-white text-[10px] font-bold px-3 py-1 rounded-full w-fit mb-2 tracking-wider`}>
            {badge}
          </span>
      ...
      <div className="p-8 flex flex-col flex-grow">
        <p className="text-[#2b4d40] mb-8 leading-relaxed font-medium">
          {description}
        </p>

        <ul className="space-y-4 mb-10 grow">
          {features.map((feature, index) => (
            <li key={index} className="flex items-center text-[#1a2e26] font-semibold">
        ...
        {/* Action Button */}
        <button className={`w-full py-4 rounded-xl font-bold text-lg transition-all ${buttonStyle}`}>
          {buttonText}
        </button>
      </div>
    </div>
